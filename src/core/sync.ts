...
export function multiAgentStep(
      ...
      if (amount < 0) {
        ...
        if (marginAccount.swfRebalancingEnabled) {
          ...
          if (marginAccount.swfRehypothecationAuthorized && marginAccount.swfRehypothecationVaultId && marginAccount.swfRehypothecationPercentage !== undefined) {
            ...
            if (vault) {
              const rehypothecatedAmount = Math.floor(potentialCollateral * (marginAccount.swfRehypothecationPercentage / 100));
              swfRehypothecationPremium = Math.round(rehypothecatedAmount * (0.10 + vault.sweepRisk));
            }
          }
        }

        const maintenanceRequirement = Math.round((0.20 * (sumCdsNotional + sumSwfCdsNotional)) + (0.10 * sumBorrowedAmount) + rehypothecationPremium + swfRehypothecationPremium);
  ...
  // Handle decentralized AUTHORIZE_SWF_MARGIN_REHYPOTHECATION action (AF-133)
  if ((action as any).type === "AUTHORIZE_SWF_MARGIN_REHYPOTHECATION") {
    const { syndicateId, vaultId, percentage, timestamp } = action as any;
    ...
    if (!syndicateId) {
      rejectionReason = `Syndicate ID is required to authorize SWF margin rehypothecation.`;
    } else if (!vaultId) {
      rejectionReason = `Vault ID is required to authorize SWF margin rehypothecation.`;
    } else if (percentage === undefined || percentage < 0 || percentage > 100 || !Number.isInteger(percentage)) {
      rejectionReason = `Rehypothecation percentage must be an integer between 0 and 100.`;
    } else if (!syndicate) {
      rejectionReason = `Syndicate ${syndicateId} does not exist.`;
      ...
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote on SWF rehypothecation.`;
    } else if (!vault) {
      rejectionReason = `Vault ${vaultId} does not exist.`;
    } else {
      ok = true;
    ...
    if (ok && syndicate && marginAccount) {
      const swfMarginRehypothecationVotes = { ...(state.swfMarginRehypothecationVotes || {}) };
      if (!swfMarginRehypothecationVotes[syndicateId]) {
        swfMarginRehypothecationVotes[syndicateId] = {};
      } else {
        swfMarginRehypothecationVotes[syndicateId] = { ...swfMarginRehypothecationVotes[syndicateId] };
      }

      const existingVote = swfMarginRehypothecationVotes[syndicateId][agentId];
      if (!existingVote || timestamp > existingVote.timestamp) {
        swfMarginRehypothecationVotes[syndicateId][agentId] = {
          vaultId,
          percentage,
          timestamp,
        };
        newState.swfMarginRehypothecationVotes = swfMarginRehypothecationVotes;
        newState = reconcileSWFMarginRehypothecations(newState, pack);

        const currentRehypo = newState.marginAccounts?.[syndicateId];
        const isAuthorized = currentRehypo?.swfRehypothecationAuthorized || false;
        const currentPercentage = currentRehypo?.swfRehypothecationPercentage ?? 0;
        const currentVaultId = currentRehypo?.swfRehypothecationVaultId ?? "";

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[SWF Margin Rehypothecation Vote] Agent ${agentId} voted to authorize SWF rehypothecation for Syndicate ${syndicateId} (Vault: ${vaultId}, Percentage: ${percentage}%). Consensus authorized: ${isAuthorized} (Vault: ${currentVaultId}, Percentage: ${currentPercentage}%).`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ SWF Margin rehypothecation vote cast by ${agentId} for Syndicate ${syndicateId} (${percentage}% to ${vaultId}).`,
        } as any);

        customEvents.push({
          type: "swf_margin_rehypothecation_voted" as any,
          syndicateId,
          agentId,
          vaultId,
          percentage,
  ...
  // Handle decentralized REVOKE_SWF_MARGIN_REHYPOTHECATION action (AF-133)
  if ((action as any).type === "REVOKE_SWF_MARGIN_REHYPOTHECATION") {
    const { syndicateId, timestamp } = action as any;
    ...
    if (!syndicateId) {
      rejectionReason = `Syndicate ID is required to revoke SWF margin rehypothecation.`;
    } else if (!syndicate) {
      rejectionReason = `Syndicate ${syndicateId} does not exist.`;
    } else if (!marginAccount) {
      rejectionReason = `Syndicate ${syndicateId} does not have a margin account.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote to revoke SWF rehypothecation.`;
    } else {
      ok = true;
    }
    ...
    if (ok && syndicate && marginAccount) {
      const swfMarginRehypothecationRevokeVotes = { ...(state.swfMarginRehypothecationRevokeVotes || {}) };
      if (!swfMarginRehypothecationRevokeVotes[syndicateId]) {
        swfMarginRehypothecationRevokeVotes[syndicateId] = {};
      } else {
        swfMarginRehypothecationRevokeVotes[syndicateId] = { ...swfMarginRehypothecationRevokeVotes[syndicateId] };
      }

      const existingVote = swfMarginRehypothecationRevokeVotes[syndicateId][agentId];
      if (!existingVote || timestamp > existingVote.timestamp) {
        swfMarginRehypothecationRevokeVotes[syndicateId][agentId] = {
          timestamp,
        };
        newState.swfMarginRehypothecationRevokeVotes = swfMarginRehypothecationRevokeVotes;
        newState = reconcileSWFMarginRehypothecations(newState, pack);
        ...
        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[SWF Margin Rehypothecation Revoke Vote] Agent ${agentId} voted to revoke SWF rehypothecation for Syndicate ${syndicateId}. Consensus authorized: ${isAuthorized}.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ SWF Margin rehypothecation revoke vote cast by ${agentId} for Syndicate ${syndicateId}.`,
        } as any);

        customEvents.push({
          type: "swf_margin_rehypothecation_revoke_voted" as any,
          syndicateId,
          agentId,
          timestamp,
        });
  ...
  // Handle decentralized SET_SWF_MARGIN_REBALANCING_POLICY action (AF-133)
  if ((action as any).type === "SET_SWF_MARGIN_REBALANCING_POLICY") {
    const { syndicateId, enabled, vaultTargets, liquidityBufferRatio, bufferTriggerRatio, timestamp } = action as any;
    ...
    if (!syndicateId) {
      rejectionReason = `Syndicate ID is required to set SWF margin rebalancing policy.`;
    } else if (enabled === undefined) {
      rejectionReason = `Enabled status is required to set SWF margin rebalancing policy.`;
    } else if (enabled && (!vaultTargets || Object.keys(vaultTargets).length === 0)) {
      rejectionReason = `Vault targets are required when SWF rebalancing is enabled.`;
    } else if (liquidityBufferRatio === undefined || liquidityBufferRatio < 0 || liquidityBufferRatio > 100 || !Number.isInteger(liquidityBufferRatio)) {
      rejectionReason = `Liquidity buffer ratio must be an integer between 0 and 100.`;
    } else if (bufferTriggerRatio === undefined || bufferTriggerRatio <= 0) {
      rejectionReason = `Buffer trigger ratio must be greater than 0.`;
      ...
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote on SWF rebalancing policy.`;
    } else {
      let allVaultsExist = true;
      let sumTargets = 0;
      for (const [vaultId, pct] of Object.entries(vaultTargets || {})) {
      ...
      if (allVaultsExist) {
        if (enabled && sumTargets !== 100) {
          rejectionReason = `Sum of vault target percentages must equal exactly 100 when SWF rebalancing is enabled.`;
        } else {
          ok = true;
        }
      }
    ...
    if (ok && syndicate && marginAccount) {
      const swfMarginRebalancingPolicyVotes = { ...(state.swfMarginRebalancingPolicyVotes || {}) };
      if (!swfMarginRebalancingPolicyVotes[syndicateId]) {
        swfMarginRebalancingPolicyVotes[syndicateId] = {};
      } else {
        swfMarginRebalancingPolicyVotes[syndicateId] = { ...swfMarginRebalancingPolicyVotes[syndicateId] };
      }

      const existingVote = swfMarginRebalancingPolicyVotes[syndicateId][agentId];
      if (!existingVote || timestamp > existingVote.timestamp) {
        swfMarginRebalancingPolicyVotes[syndicateId][agentId] = {
          enabled,
          vaultTargets,
          liquidityBufferRatio,
          bufferTriggerRatio,
        ...
        newState.swfMarginRebalancingPolicyVotes = swfMarginRebalancingPolicyVotes;
        newState = reconcileSWFMarginRebalancingPolicies(newState, pack);

        const currentMA = newState.marginAccounts?.[syndicateId];
        const isEnabled = currentMA?.swfRebalancingEnabled || false;

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[SWF Margin Rebalancing Policy Vote] Agent ${agentId} voted on SWF rebalancing policy for Syndicate ${syndicateId}. Consensus enabled: ${isEnabled}.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ SWF Margin rebalancing policy vote cast by ${agentId} for Syndicate ${syndicateId}.`,
        } as any);

        customEvents.push({
          type: "swf_margin_rebalancing_policy_voted" as any,
          syndicateId,
          agentId,
          enabled,
          vaultTargets,
  ...
  // Handle decentralized SET_SWF_STAKING_POLICY action (AF-136)
  if ((action as any).type === "SET_SWF_STAKING_POLICY") {
    const { syndicateId, enabled, stakedFactions, timestamp } = action as any;
    ...
    if (!syndicateId) {
      rejectionReason = `Syndicate ID is required to set SWF staking policy.`;
    } else if (enabled === undefined) {
      rejectionReason = `Enabled status is required to set SWF staking policy.`;
    } else if (enabled && (!stakedFactions || Object.keys(stakedFactions).length === 0)) {
      rejectionReason = `Staked factions targets are required when SWF staking is enabled.`;
    } else if (!syndicate) {
      rejectionReason = `Syndicate ${syndicateId} does not exist.`;
    } else if (!marginAccount) {
      rejectionReason = `Syndicate ${syndicateId} does not have a margin account.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote on SWF staking policy.`;
    } else {
      let sumPct = 0;
      let validPcts = true;
      for (const [factionId, pct] of Object.entries(stakedFactions || {})) {
        if (!factionId) {
          validPcts = false;
          rejectionReason = `Faction ID cannot be empty.`;
          break;
        }
        if (typeof pct !== "number" || pct < 0 || pct > 100 || !Number.isInteger(pct)) {
          validPcts = false;
          rejectionReason = `Staking target percentage for faction ${factionId} must be a non-negative integer up to 100.`;
          break;
        }
        sumPct += pct;
      }
      if (validPcts) {
        if (sumPct > 100) {
          rejectionReason = `Total SWF staking target percentage (${sumPct}%) cannot exceed 100%.`;
        } else {
          ok = true;
        }
      }
    ...
    if (ok && syndicate && marginAccount) {
      const swfStakingPolicyVotes = { ...(state.swfStakingPolicyVotes || {}) };
      if (!swfStakingPolicyVotes[syndicateId]) {
        swfStakingPolicyVotes[syndicateId] = {};
      } else {
        swfStakingPolicyVotes[syndicateId] = { ...swfStakingPolicyVotes[syndicateId] };
      }

      const existingVote = swfStakingPolicyVotes[syndicateId][agentId];
      if (!existingVote || timestamp > existingVote.timestamp) {
        swfStakingPolicyVotes[syndicateId][agentId] = {
          enabled,
          stakedFactions,
          timestamp,
        };
        newState.swfStakingPolicyVotes = swfStakingPolicyVotes;
        newState = reconcileSWFStakingPolicies(newState, pack);

        const currentMA = newState.marginAccounts?.[syndicateId];
        const isEnabled = currentMA?.swfStakingEnabled || false;

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[SWF Staking Policy Vote] Agent ${agentId} voted on SWF staking policy for Syndicate ${syndicateId}. Consensus enabled: ${isEnabled}.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ SWF Staking policy vote cast by ${agentId} for Syndicate ${syndicateId}.`,
        } as any);

        customEvents.push({
          type: "swf_staking_policy_voted" as any,
          syndicateId,
          voterId: agentId,
          enabled,
          stakedFactions,
  ...
  // Handle decentralized REBALANCE_SWF_MARGIN_COLLATERAL action (AF-133)
  if ((action as any).type === "REBALANCE_SWF_MARGIN_COLLATERAL") {
    const { syndicateId, timestamp } = action as any;
    ...
    if (!syndicateId) {
      rejectionReason = `Syndicate ID is required to rebalance SWF margin collateral.`;
    } else if (!syndicate) {
      rejectionReason = `Syndicate ${syndicateId} does not exist.`;
    } else if (!marginAccount) {
      rejectionReason = `Syndicate ${syndicateId} does not have a margin account.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot trigger SWF manual rebalancing.`;
    } else if (!marginAccount.swfRebalancingEnabled) {
      rejectionReason = `SWF Rebalancing policy is not enabled for Syndicate ${syndicateId}.`;
    } else {
      ok = true;
    }
    ...
    if (ok && syndicate && marginAccount) {
      const ma = { ...marginAccount };
      const collateral = ma.collateral;
      const targetBuffer = Math.floor(collateral * ((ma.swfLiquidityBufferRatio ?? 0) / 100));
      const targetRehypothecated = collateral - targetBuffer;
      const vaultAllocations: Record<string, number> = {};

      for (const [vaultId, pct] of Object.entries(ma.swfVaultTargets || {})) {
        vaultAllocations[vaultId] = Math.floor(targetRehypothecated * (pct / 100));
      }
      const sumAllocated = Object.values(vaultAllocations).reduce((a, b) => a + b, 0);
      ma.swfLiquidityBuffer = collateral - sumAllocated;
      ma.swfVaultAllocations = vaultAllocations;
      ma.timestamp = Math.max(timestamp, state.step);
      ...
      if (!newState.journal) newState.journal = [];
      newState.journal.push(
        `[SWF Margin Rebalance] Manually rebalanced SWF collateral for Syndicate ${syndicateId}. Local buffer: ${ma.swfLiquidityBuffer}, vault allocations: ${JSON.stringify(vaultAllocations)}.`
      );

      customEvents.push({
        type: "narration",
        text: `⚖️ SWF Margin collateral rebalanced for Syndicate ${syndicateId}.`,
      } as any);

      customEvents.push({
        type: "swf_margin_collateral_rebalanced" as any,
        syndicateId,
        agentId,
        timestamp,
      });
  ...
  // Handle decentralized DEPLOY_SWF_REBALANCING_ADVISOR action (AF-133)
  if ((action as any).type === "DEPLOY_SWF_REBALANCING_ADVISOR") {
    const { syndicateId, enabled, timestamp } = action as any;
    ...
    if (!syndicateId) {
      rejectionReason = `Syndicate ID is required to deploy SWF rebalancing advisor.`;
    } else if (enabled === undefined) {
      rejectionReason = `Enabled status is required to deploy SWF rebalancing advisor.`;
    } else if (!syndicate) {
      rejectionReason = `Syndicate ${syndicateId} does not exist.`;
    } else if (!marginAccount) {
      rejectionReason = `Syndicate ${syndicateId} does not have a margin account.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote on SWF rebalancing advisor.`;
    } else {
      ok = true;
    }
    ...
    if (ok && syndicate && marginAccount) {
      const swfRebalancingAdvisorVotes = { ...(state.swfRebalancingAdvisorVotes || {}) };
      if (!swfRebalancingAdvisorVotes[syndicateId]) {
        swfRebalancingAdvisorVotes[syndicateId] = {};
      } else {
        swfRebalancingAdvisorVotes[syndicateId] = { ...swfRebalancingAdvisorVotes[syndicateId] };
      }

      const existingVote = swfRebalancingAdvisorVotes[syndicateId][agentId];
      if (!existingVote || timestamp > existingVote.timestamp) {
        swfRebalancingAdvisorVotes[syndicateId][agentId] = {
          enabled,
          timestamp,
        };
        newState.swfRebalancingAdvisorVotes = swfRebalancingAdvisorVotes;
        newState = reconcileSWFRebalancingAdvisors(newState, pack);

        const currentMA = newState.marginAccounts?.[syndicateId];
        const isEnabled = currentMA?.swfAdvisorEnabled || false;

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[SWF Rebalancing Advisor Vote] Agent ${agentId} voted on deploying SWF advisor for Syndicate ${syndicateId}. Consensus enabled: ${isEnabled}.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ SWF Rebalancing advisor vote cast by ${agentId} for Syndicate ${syndicateId}.`,
        } as any);

        customEvents.push({
          type: "swf_rebalancing_advisor_voted" as any,
          syndicateId,
          agentId,
          enabled,
          timestamp,
  ...
  // Handle decentralized SET_SWF_ADVISOR_SAFETY_THRESHOLD action (AF-133)
  if ((action as any).type === "SET_SWF_ADVISOR_SAFETY_THRESHOLD") {
    const { syndicateId, threshold, timestamp } = action as any;
    ...
    if (!syndicateId) {
      rejectionReason = `Syndicate ID is required to set SWF advisor safety threshold.`;
    } else if (threshold === undefined || threshold < 0) {
      rejectionReason = `Valid threshold is required to set SWF advisor safety threshold.`;
    } else if (!syndicate) {
      rejectionReason = `Syndicate ${syndicateId} does not exist.`;
    } else if (!marginAccount) {
      rejectionReason = `Syndicate ${syndicateId} does not have a margin account.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote on SWF advisor safety threshold.`;
    } else {
      ok = true;
    }
    ...
    if (ok && syndicate && marginAccount) {
      const swfAdvisorSafetyThresholdVotes = { ...(state.swfAdvisorSafetyThresholdVotes || {}) };
      if (!swfAdvisorSafetyThresholdVotes[syndicateId]) {
        swfAdvisorSafetyThresholdVotes[syndicateId] = {};
      } else {
        swfAdvisorSafetyThresholdVotes[syndicateId] = { ...swfAdvisorSafetyThresholdVotes[syndicateId] };
      }

      const existingVote = swfAdvisorSafetyThresholdVotes[syndicateId][agentId];
      if (!existingVote || timestamp > existingVote.timestamp) {
        swfAdvisorSafetyThresholdVotes[syndicateId][agentId] = {
          threshold,
          timestamp,
        };
        newState.swfAdvisorSafetyThresholdVotes = swfAdvisorSafetyThresholdVotes;
        newState = reconcileSWFAdvisorSafetyThresholds(newState, pack);

        const currentMA = newState.marginAccounts?.[syndicateId];
        const isThreshold = currentMA?.swfAdvisorSafetyThreshold ?? 0;

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[SWF Advisor Safety Threshold Vote] Agent ${agentId} voted on SWF advisor safety threshold for Syndicate ${syndicateId}. Consensus threshold: ${isThreshold}%.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ SWF Advisor safety threshold vote cast by ${agentId} for Syndicate ${syndicateId}.`,
        } as any);

        customEvents.push({
          type: "swf_advisor_safety_threshold_voted" as any,
          syndicateId,
          agentId,
          threshold,
          timestamp,
  ...
  // Handle decentralized PROPOSE_SOVEREIGN_BOND action (AF-123)
  if ((action as any).type === "PROPOSE_SOVEREIGN_BOND") {
    ...
    if (!proposalId) {
      rejectionReason = `Proposal ID is required to propose a sovereign bond.`;
    } else if (!syndicateId) {
      rejectionReason = `Syndicate ID is required.`;
    } else if (!factionId) {
      rejectionReason = `Faction ID is required.`;
    } else if (faceValue === undefined || faceValue <= 0) {
      rejectionReason = `Face value must be positive.`;
    } else if (interestRate === undefined || interestRate < 0) {
      rejectionReason = `Interest rate must be non-negative.`;
    } else if (termEpochs === undefined || termEpochs <= 0) {
      rejectionReason = `Term epochs must be positive.`;
    } else if (!syndicate) {
      rejectionReason = `Syndicate ${syndicateId} does not exist.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot propose a sovereign bond.`;
    } else {
      // Validate faction reserves
      const factionReserve = state.factionReservePools?.[factionId] ?? 10000;
      if (factionReserve < faceValue) {
        rejectionReason = `Faction reserve for ${factionId} is insufficient to purchase the bond (requires ${faceValue}, has ${factionReserve}).`;
      } else {
        // Validate faction reputation/standing
        const standing = getSyndicateFactionStanding(state, syndicateId, factionId);
        if (standing < 50) {
          rejectionReason = `Syndicate standing with faction ${factionId} is too low to issue a bond (requires at least 50, has ${standing}).`;
        } else {
          ok = true;
        }
      }
    ...
    if (ok && syndicate) {
      const proposals = { ...(state.sovereignDebtProposals || {}) };
      const existingProposal = proposals[proposalId];
      if (!existingProposal || timestamp > existingProposal.timestamp) {
        const votes = existingProposal?.votes ? { ...existingProposal.votes } : {};
        votes[agentId] = { vote: true, timestamp };
        ...
        newState.sovereignDebtProposals = proposals;
        newState = reconcileSovereignBonds(newState, pack);

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[Sovereign Bond Proposed] Agent ${agentId} proposed sovereign bond issuance ${proposalId} with faction ${factionId} for ${faceValue} gold at ${interestRate}% for ${termEpochs} epochs.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ Sovereign bond proposal created by ${agentId} for faction ${factionId} (face value: ${faceValue} gold, rate: ${interestRate}%, term: ${termEpochs} epochs).`,
        } as any);

        customEvents.push({
          type: "sovereign_bond_proposed" as any,
          proposalId,
          syndicateId,
          agentId,
          factionId,
  ...
  // Handle decentralized VOTE_SOVEREIGN_BOND action (AF-123)
  if ((action as any).type === "VOTE_SOVEREIGN_BOND") {
    const { syndicateId, proposalId, vote, timestamp } = action as any;
    ...
    if (!syndicateId) {
      ...
      rejectionReason = `Sovereign bond proposal ${proposalId} does not exist.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote on sovereign bond proposal.`;
    } else {
      ok = true;
    }
    ...
    if (ok && syndicate && proposal) {
      const proposalsCopy = { ...(state.sovereignDebtProposals || {}) };
      const currentProp = { ...proposalsCopy[proposalId] };
      const votes = currentProp.votes ? { ...currentProp.votes } : {};
      ...
      if (!existingVote || timestamp > existingVote.timestamp) {
        ...
        newState.sovereignDebtProposals = proposalsCopy;
        newState = reconcileSovereignBonds(newState, pack);

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[Sovereign Bond Voted] Agent ${agentId} voted ${vote ? "FOR" : "AGAINST"} sovereign bond proposal ${proposalId}.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ Sovereign bond vote cast by ${agentId} for proposal ${proposalId}.`,
        } as any);
  ...
  // Handle decentralized PROPOSE_SOVEREIGN_WEALTH_FUND action (AF-128)
  if ((action as any).type === "PROPOSE_SOVEREIGN_WEALTH_FUND") {
    const { proposalId, fundId, syndicateId, amount, timestamp } = action as any;
    ...
    if (!proposalId) {
      ...
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot propose pooling reserves.`;
    } else if ((syndicate.warChest ?? 0) < amount) {
      rejectionReason = `Syndicate ${syndicateId} has insufficient reserves in its warChest (requires ${amount}, has ${syndicate.warChest ?? 0}).`;
    } else {
      ok = true;
    }
    ...
    if (ok && syndicate) {
      const proposals = { ...(state.sovereignWealthFundProposals || {}) };
      const existingProposal = proposals[proposalId];
      if (!existingProposal || timestamp > existingProposal.timestamp) {
        const votes = existingProposal?.votes ? { ...existingProposal.votes } : {};
        votes[agentId] = { vote: true, timestamp };
        ...
        newState.sovereignWealthFundProposals = proposals;
        newState = reconcileSovereignWealthFunds(newState, pack);

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[SWF Proposed] Agent ${agentId} proposed reserve pooling ${proposalId} of ${amount} gold from syndicate ${syndicateId} into SWF ${fundId}.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ Sovereign Wealth Fund pooling proposed by ${agentId}: Syndicate ${syndicateId} requests to pool ${amount} gold into SWF ${fundId}.`,
        } as any);

        customEvents.push({
          type: "swf_proposed" as any,
          proposalId,
          fundId,
          syndicateId,
          amount,
  ...
  // Handle decentralized VOTE_SOVEREIGN_WEALTH_FUND action (AF-128)
  if ((action as any).type === "VOTE_SOVEREIGN_WEALTH_FUND") {
    const { proposalId, syndicateId, vote, timestamp } = action as any;
    ...
    if (!syndicateId) {
      ...
      rejectionReason = `SWF proposal ${proposalId} does not exist.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote on SWF proposal.`;
    } else if (proposal.syndicateId !== syndicateId) {
      rejectionReason = `Syndicate ${syndicateId} is not the proposing syndicate for proposal ${proposalId} and cannot vote on it.`;
    } else {
      ok = true;
    }
    ...
    if (ok && syndicate && proposal) {
      const proposalsCopy = { ...(state.sovereignWealthFundProposals || {}) };
      const currentProp = { ...proposalsCopy[proposalId] };
      const votes = currentProp.votes ? { ...currentProp.votes } : {};
      ...
      if (!existingVote || timestamp > existingVote.timestamp) {
        ...
        newState.sovereignWealthFundProposals = proposalsCopy;
        newState = reconcileSovereignWealthFunds(newState, pack);

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[SWF Voted] Agent ${agentId} in syndicate ${syndicateId} voted ${vote ? "YES" : "NO"} on SWF pooling proposal ${proposalId}.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ Vote cast by ${agentId} in syndicate ${syndicateId} on SWF pooling proposal ${proposalId}: ${vote ? "YES" : "NO"}.`,
        } as any);

        customEvents.push({
          type: "swf_voted" as any,
          proposalId,
          syndicateId,
          agentId,
          vote,
  ...
  // Handle decentralized PROPOSE_JOINT_VENTURE_INVESTMENT action (AF-128)
  if ((action as any).type === "PROPOSE_JOINT_VENTURE_INVESTMENT") {
    const { proposalId, fundId, proposerSyndicateId, targetType, targetId, amount, yieldRate, timestamp } = action as any;
    ...
    if (!proposalId) {
      rejectionReason = `Proposal ID is required to propose a joint-venture investment.`;
    } else if (!fundId) {
      rejectionReason = `Fund ID is required.`;
    } else if (!proposerSyndicateId) {
      rejectionReason = `Proposer Syndicate ID is required.`;
    } else if (!targetType || !["FactionBond", "ArbitrageRoute"].includes(targetType)) {
      rejectionReason = `Target type must be FactionBond or ArbitrageRoute.`;
    } else if (!targetId) {
      rejectionReason = `Target ID is required.`;
    } else if (amount === undefined || amount <= 0 || !Number.isInteger(amount)) {
      rejectionReason = `Amount must be a positive integer.`;
    } else if (yieldRate === undefined || yieldRate < 0) {
      rejectionReason = `Yield rate must be non-negative.`;
    } else if (!syndicate) {
      rejectionReason = `Proposer Syndicate ${proposerSyndicateId} does not exist.`;
    } else if (!fund) {
      rejectionReason = `Sovereign Wealth Fund ${fundId} does not exist.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of proposer syndicate ${proposerSyndicateId} and cannot propose joint-venture investment.`;
    } else if (!fund.syndicates[proposerSyndicateId]) {
      rejectionReason = `Syndicate ${proposerSyndicateId} is not a participant in SWF ${fundId} and cannot propose investments.`;
    } else if (fund.totalReserves < amount) {
      rejectionReason = `SWF ${fundId} has insufficient reserves (requires ${amount}, has ${fund.totalReserves}).`;
    } else {
      ok = true;
    }
    ...
    if (ok && syndicate && fund) {
      const proposals = { ...(state.jointVentureInvestmentProposals || {}) };
      const existingProposal = proposals[proposalId];
      if (!existingProposal || timestamp > existingProposal.timestamp) {
        const votes = existingProposal?.votes ? { ...existingProposal.votes } : {};
        votes[agentId] = { vote: true, timestamp };
        ...
        newState.jointVentureInvestmentProposals = proposals;
        newState = reconcileJointVentureInvestments(newState, pack);

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[JV Proposed] Agent ${agentId} proposed joint-venture investment ${proposalId} from fund ${fundId} in ${targetType} on ${targetId} for ${amount} gold.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ Joint-Venture Investment proposed by ${agentId}: Proposes allocating ${amount} gold from SWF ${fundId} to ${targetType} ${targetId} (yield: ${yieldRate}%).`,
        } as any);

        customEvents.push({
          type: "jv_proposed" as any,
          proposalId,
          fundId,
          proposerSyndicateId,
          targetType,
  ...
  // Handle decentralized VOTE_JOINT_VENTURE_INVESTMENT action (AF-128)
  if ((action as any).type === "VOTE_JOINT_VENTURE_INVESTMENT") {
    const { proposalId, syndicateId, vote, timestamp } = action as any;
    ...
    if (!syndicateId) {
      ...
      rejectionReason = `Joint-Venture proposal ${proposalId} does not exist.`;
    } else if (!syndicate.members.includes(agentId)) {
      rejectionReason = `Agent ${agentId} is not a member of syndicate ${syndicateId} and cannot vote on joint-venture proposal.`;
    } else {
    ...
    if (ok && syndicate && proposal) {
      const proposalsCopy = { ...(state.jointVentureInvestmentProposals || {}) };
      const currentProp = { ...proposalsCopy[proposalId] };
      const votes = currentProp.votes ? { ...currentProp.votes } : {};
      ...
      if (!existingVote || timestamp > existingVote.timestamp) {
        ...
        newState.jointVentureInvestmentProposals = proposalsCopy;
        newState = reconcileJointVentureInvestments(newState, pack);

        if (!newState.journal) newState.journal = [];
        newState.journal.push(
          `[JV Voted] Agent ${agentId} in syndicate ${syndicateId} voted ${vote ? "YES" : "NO"} on JV investment proposal ${proposalId}.`
        );

        customEvents.push({
          type: "narration",
          text: `🗳️ Vote cast by ${agentId} in syndicate ${syndicateId} on JV investment proposal ${proposalId}: ${vote ? "YES" : "NO"}.`,
        } as any);
