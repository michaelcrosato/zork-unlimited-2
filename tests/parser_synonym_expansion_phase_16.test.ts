...
describe("Parser Synonym Expansion Phase 16 (Task-F53)", () => {
  const actions = [
  ...
  it("should map newly added movement verbs to MOVE action", () => {
    ...
    expect(mapCommand("wend one's way to the east", actions).action).toEqual({ type: "MOVE", direction: "east" });
    expect(mapCommand("wend ones way to east", actions).action).toEqual({ type: "MOVE", direction: "east" });
    expect(mapCommand("wend ones way to the east", actions).action).toEqual({ type: "MOVE", direction: "east" });
  });
  ...
  it("should map newly added inspection verbs to LOOK action", () => {
    ...
    expect(mapCommand("peer closely at the altar", actions).action).toEqual({ type: "LOOK", target: "altar" });
  });
  ...
  it("should map newly added take verbs to TAKE action", () => {
    ...
    expect(mapCommand("help oneself to the katana", actions).action).toEqual({ type: "TAKE", item: "katana" });
    expect(mapCommand("help ones self to katana", actions).action).toEqual({ type: "TAKE", item: "katana" });
    expect(mapCommand("help ones self to the katana", actions).action).toEqual({ type: "TAKE", item: "katana" });
    expect(mapCommand("make off with katana", actions).action).toEqual({ type: "TAKE", item: "katana" });
    expect(mapCommand("make off with the katana", actions).action).toEqual({ type: "TAKE", item: "katana" });
  });

  it("should map newly added drop verbs to DROP action", () => {
    expect(mapCommand("part with boots", actions).action).toEqual({ type: "DROP", item: "boots" });
    expect(mapCommand("part with the boots", actions).action).toEqual({ type: "DROP", item: "boots" });
    ...
    expect(mapCommand("relinquish control of boots", actions).action).toEqual({ type: "DROP", item: "boots" });
    expect(mapCommand("relinquish control of the boots", actions).action).toEqual({ type: "DROP", item: "boots" });
  });
  ...
  it("should map newly added open/close verbs to OPEN/CLOSE action", () => {
    ...
    expect(mapCommand("slide open the vault", actions).action).toEqual({ type: "OPEN", target: "vault" });
    expect(mapCommand("seal shut door", actions).action).toEqual({ type: "CLOSE", target: "door" });
    expect(mapCommand("seal shut the door", actions).action).toEqual({ type: "CLOSE", target: "door" });
    expect(mapCommand("swing shut door", actions).action).toEqual({ type: "CLOSE", target: "door" });
    expect(mapCommand("swing shut the door", actions).action).toEqual({ type: "CLOSE", target: "door" });
  });

  it("should map newly added unlock verbs to UNLOCK action", () => {
    expect(mapCommand("crack the lock on chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
    expect(mapCommand("crack the lock on the chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
    expect(mapCommand("deactivate the lock on chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
    expect(mapCommand("deactivate the lock on the chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
    expect(mapCommand("clear the lock on chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
    expect(mapCommand("clear the lock on the chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
  });
  ...
  it("should map newly added use verbs to USE action", () => {
    ...
    expect(mapCommand("put to work the lockpick", actions).action).toEqual({ type: "USE", target: "chest" });
  });

  it("should map newly added combat verbs to FIGHT action", () => {
    expect(mapCommand("assault ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
    expect(mapCommand("assault the ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
    expect(mapCommand("initiate combat with ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
    expect(mapCommand("initiate combat with the ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
    expect(mapCommand("launch an attack on ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
    expect(mapCommand("launch an attack on the ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
  ...
  it("should map newly added dialogue verbs to TALK action", () => {
    ...
    expect(mapCommand("engage in dialogue with capo", actions).action).toEqual({ type: "TALK", npc: "capo" });
    expect(mapCommand("engage in dialogue with the capo", actions).action).toEqual({ type: "TALK", npc: "capo" });
  });
});
