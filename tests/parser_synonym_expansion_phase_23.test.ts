...
describe("Parser Synonym Expansion Phase 23 (Task-F60)", () => {
  ...
  it("should map newly added movement verbs to MOVE action", () => {
    ...
    expect(mapCommand("direct one's course towards the east", actions).action).toEqual({ type: "MOVE", direction: "east" });
    expect(mapCommand("direct ones course towards east", actions).action).toEqual({ type: "MOVE", direction: "east" });
    expect(mapCommand("direct ones course towards the east", actions).action).toEqual({ type: "MOVE", direction: "east" });
    expect(mapCommand("navigate a route towards east", actions).action).toEqual({ type: "MOVE", direction: "east" });
    expect(mapCommand("navigate a route towards the east", actions).action).toEqual({ type: "MOVE", direction: "east" });
    expect(mapCommand("journey in the direction of east", actions).action).toEqual({ type: "MOVE", direction: "east" });
    expect(mapCommand("journey in the direction of the east", actions).action).toEqual({ type: "MOVE", direction: "east" });
  });

  it("should map newly added inspection verbs to LOOK action", () => {
    expect(mapCommand("carry out an inspection of altar", actions).action).toEqual({ type: "LOOK", target: "altar" });
    expect(mapCommand("carry out an inspection of the altar", actions).action).toEqual({ type: "LOOK", target: "altar" });
    ...
    expect(mapCommand("perform a visual check on the altar", actions).action).toEqual({ type: "LOOK", target: "altar" });
  });
  ...
  it("should map newly added take verbs to TAKE action", () => {
    ...
    expect(mapCommand("bring under one's custody the katana", actions).action).toEqual({ type: "TAKE", item: "katana" });
  ...
  it("should map newly added unlock verbs to UNLOCK action", () => {
    ...
    expect(mapCommand("deactivate security systems on chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
    expect(mapCommand("deactivate security systems on the chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
    expect(mapCommand("bypass security systems on chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
    expect(mapCommand("bypass security systems on the chest", actions).action).toEqual({ type: "UNLOCK", target: "chest" });
  });

  it("should map newly added use verbs to USE action", () => {
    expect(mapCommand("harness the functionality of lockpick", actions).action).toEqual({ type: "USE", target: "chest" });
    expect(mapCommand("harness the functionality of the lockpick", actions).action).toEqual({ type: "USE", target: "chest" });
    ...
    expect(mapCommand("make use of the functions of lockpick", actions).action).toEqual({ type: "USE", target: "chest" });
    expect(mapCommand("make use of the functions of the lockpick", actions).action).toEqual({ type: "USE", target: "chest" });
  });
  ...
  it("should map newly added combat verbs to FIGHT action", () => {
    ...
    expect(mapCommand("commence hostilities against the ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
    expect(mapCommand("initiate an assault against ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
    expect(mapCommand("initiate an assault against the ghoul", actions).action).toEqual({ type: "FIGHT", npc: "ghoul" });
  });
