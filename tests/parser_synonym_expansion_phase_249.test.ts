...
describe("Parser Synonym Expansion Phase 249 (Task-F287)", () => {
  ...
  it("should map newly added movement verbs to MOVE action", () => {
    expect(mapCommand("navigate one's vector of promegakaryoblastopoiesis towards the location of east", actions).action).toEqual({
      type: "MOVE",
      direction: "east",
    });
    expect(mapCommand("steer one's vector of promegakaryoblastopoiesis in the direction of the coordinates of east", actions).action).toEqual({
      type: "MOVE",
      direction: "east",
    });
    expect(mapCommand("direct ones vector of promegakaryoblastopoiesis towards the coordinates of the location of east", actions).action).toEqual({
      type: "MOVE",
      direction: "east",
    });
  });
  ...
  it("should map newly added unlock verbs to UNLOCK action", () => {
    ...
    expect(mapCommand("disengage the primary geographer and cartographer security device on chest", actions).action).toEqual({
      type: "UNLOCK",
      target: "chest",
    });
  });
