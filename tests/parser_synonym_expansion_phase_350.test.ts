...
describe("Parser Synonym Expansion Phase 350 (Task-F388)", () => {
  ...
  it("should map newly added movement verbs to MOVE action", () => {
    ...
    expect(
      mapCommand("direct ones vector of microgliocytoclastopoiesis towards the coordinates of the location of east", actions)
        .action
    ).toEqual({
      type: "MOVE",
      direction: "east",
  ...
  it("should map newly added open/close verbs to OPEN/CLOSE action", () => {
    ...
    expect(mapCommand("force completely and microgliocytoclasto-rheologically wide open vault", actions).action).toEqual({
      type: "OPEN",
      target: "vault",
    });
    expect(mapCommand("force completely and microgliocytoclasto rheologically wide open vault", actions).action).toEqual({
  ...
  it("should map newly added unlock verbs to UNLOCK action", () => {
    expect(mapCommand("deactivate all microgliocytoclastologist and microglioblastoclastologist security devices of chest", actions).action).toEqual({
      type: "UNLOCK",
      target: "chest",
    });
    expect(mapCommand("bypass all microgliocytoclastologists and microglioblastoclastologists security devices on chest", actions).action).toEqual({
      type: "UNLOCK",
      target: "chest",
    });
    expect(
      mapCommand("disengage the primary microgliocytoclastologist and microglioblastoclastologist security device on chest", actions)
        .action
    ).toEqual({
      type: "UNLOCK",
      target: "chest",
  ...
  it("should map newly added dialogue verbs to TALK action", () => {
    ...
    expect(mapCommand("engage in a/an microgliocytoclastopathological face-to-face discussion with capo", actions).action).toEqual({
      type: "TALK",
      npc: "capo",
    });
    expect(mapCommand("strike up an microgliocytoclastopathological facetoface discussion with capo", actions).action).toEqual({
      type: "TALK",
      npc: "capo",
    });
  });
