...
describe("Sensory Narration Variety & Custom Categories", () => {
  it("should categorize and select from larger pools for different room IDs", () => {
    ...
    expect(desc).toMatch(
      /pine needles|owl|loam|watchfulness|leaves crunch|green shadows|cedar|mournful sigh|woodland creature|moss|twig|sunlight|wildflowers|bird|crickets|insects|spiderwebs|jasmine|pine cones|spongy|spring|pockets|rotten log/i
    );
    ...
    expect(desc).toMatch(
      /battlements|weapons|smoke|arrow loops|oak beams|soot|banners|iron-banded|crest|sentries|sconce|wooden beams|leather|whistling|footprints|iron rings|tethering|floorboards|arrow slits|bright blades|smell of oil|horizon|scratch marks|drafty and brisk|buckle|armor|iron bolts|window shutters|utility/i
    );
    ...
    expect(desc).toMatch(
      /woodsmoke|carriage ruts|signpost|civilization|tavern|rag|insects|lantern light|dust|safety|cart wheel|chimes|worn path|grass|birds nesting|eaves|freshly cut hay|damp straw|hoofprints|chimney|cedarwood|hammer|anvil|wild herbs|mint|discarded straw|twigs|hitching horses|cattle|laundry/i
    );
  });
