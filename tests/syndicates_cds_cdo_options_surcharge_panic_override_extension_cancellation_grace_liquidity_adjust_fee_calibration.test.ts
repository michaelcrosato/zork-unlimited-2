...
describe("Syndicate CDO Options Panic Override Threshold Adjust Fee Calibration (AF-261)", () => {
  ...
  it("should support proposing, voting, and authorizing dynamic fee calibrations", () => {
    ...
    expect(
      state.cdsCdoYieldHedgingOptionSurchargePanicOverrideExtensionCancellationGraceLiquidityAdjustProposalFee
    ).toBe(500);
    expect(state.cdsCdoYieldHedgingOptionSurchargePanicOverrideExtensionCancellationGraceLiquidityAdjustVoteFee).toBe(
      100
    );
  ...
  it("should merge fee calibration proposals and votes correctly via Gossip mesh node convergence", () => {
    ...
    expect(
      mergedState.cdsCdoYieldHedgingOptionSurchargePanicOverrideExtensionCancellationGraceLiquidityAdjustProposalFee
    ).toBe(650);
    expect(
      mergedState.cdsCdoYieldHedgingOptionSurchargePanicOverrideExtensionCancellationGraceLiquidityAdjustVoteFee
    ).toBe(150);
  });
  ...
  it("should validate dynamic fee calibration scaling in tickEconomy under spamming and heat (AF-261)", () => {
    ...
    expect(
      state.cdsCdoYieldHedgingOptionSurchargePanicOverrideExtensionCancellationGraceLiquidityAdjustBaseProposalFee
    ).toBe(600);
    expect(
      state.cdsCdoYieldHedgingOptionSurchargePanicOverrideExtensionCancellationGraceLiquidityAdjustBaseVoteFee
    ).toBe(150);

    // Dynamic fee scaling in tickEconomy:
    let ticked = tickEconomy(state, mockPack);
    expect(
      ticked.cdsCdoYieldHedgingOptionSurchargePanicOverrideExtensionCancellationGraceLiquidityAdjustProposalFee
    ).toBe(600);
    expect(ticked.cdsCdoYieldHedgingOptionSurchargePanicOverrideExtensionCancellationGraceLiquidityAdjustVoteFee).toBe(
      150
    );
