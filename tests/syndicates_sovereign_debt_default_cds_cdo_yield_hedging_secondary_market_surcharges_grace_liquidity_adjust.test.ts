...
describe("CDO Surcharge Grace Period Minimum Liquidity Threshold Adjustment (AF-260)", () => {
  const mockPack: ParserPack = ParserPackSchema.parse({
    meta: {
      id: "yield_hedging_surcharges_grace_liquidity_adjust_test_pack",
      title: "Yield Hedging Surcharges Grace Liquidity Adjust Test Pack",
      start_room: "vault",
  ...
  it("should validate and reconcile proposing, voting, and authorizing a surcharge grace period minimum liquidity threshold adjustment", () => {
    ...
    expect(graceProp!.minLiquidityThreshold).toBe(800);
  });

  it("should enforce the adjusted threshold in tickEconomy, cancelling grace and override when reserves drop below new threshold", () => {
    let state = setupState();
