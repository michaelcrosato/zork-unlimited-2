...
describe("Parser Synonym Expansion (Cycle #415 / Mock client isolation & Synonyms)", () => {
  ...
  it("should map dig mound with shovel to use rusty shovel on earthen mound", () => {
    expect(mapCommand("dig mound with shovel", actions).action).toEqual({ type: "USE", item: "shovel", target: "mound" });
    expect(mapCommand("dig earth with shovel", actions).action).toEqual({ type: "USE", item: "shovel", target: "mound" });
  });
